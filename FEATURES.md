...
| `mw-menu-fetch` | | Fetch current menu hierarchy and executable UI actions. `menu` is user interface such as inventory. `help` is overlay such as tooltips. | <ul><li>`menu_id` (number, optional) - Fetch a non-root hierarchy by ID.</li><li>`menu_name` (string, optional) - Fetch a non-root hierarchy by name.</li><li>`output_mode` (string, optional) - Return `tree`, `actions`, or `both` (default).</li></ul> | <ul><li>`actions` (array) - Flat executable actions with mainRoot-relative paths. Inventory item tiles include `inventory_tile` metadata.</li><li>`cursor_tile` (object) - Current cursor item when present.</li><li>`help` (object)</li><li>`menu` (object)</li></ul> | <ul><li>`readOnlyHint`: true</li></ul> |
