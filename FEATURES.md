...
| `mw-menu-action` | | Action to a non-root menu. | <ul><li>`action` (string, required) - Action to perform on the menu.</li><li>`menu_id` (number, optional) - Action by menu ID; specify exactly one selector.</li><li>`menu_name` (string, optional) - Action by menu name; specify exactly one selector.</li><li>`menu_path` (string, optional) - Action using a path returned by `mw-menu-fetch`; specify exactly one selector.</li><li>`text` (string, optional) - Text to input if action is `textInput`.</li></ul> | | |
...
| `mw-player-navigate` | | Navigate the player character through the game world toward an intentional destination, such as an observed NPC, door, reference, location, or world-space coordinate. This is useful for travel when a reachable destination is known. Navigation returns route node and waypoint counts; verify arrival with player, reference, or world state. Use cancel_navigation to stop an active route. | <ul><li>`action` (string, required) - Method to navigate the player character or cancel active navigation.</li><li>`position_x` (number, optional) - Destination X coordinate in world space.</li><li>`position_y` (number, optional) - Destination Y coordinate in world space.</li><li>`position_z` (number, optional) - Destination Z coordinate in world space.</li><li>`cell_id` (string, optional) - Optional cell containing the destination.</li></ul> | <ul><li>`route_node_count` (number) - Number of pathgrid nodes in the started route.</li><li>`waypoint_count` (number) - Number of waypoints in the started route.</li></ul> | |
| `mw-reference-fetch` | | Fetch references in current active cells. | <ul><li>`category` (array, optional) - Filter by activators, actors, or statics.</li><li>`detail_level` (string, optional) - Serialization detail: minimal, standard, or full.</li><li>`id` (string, optional) - Filter references by ID.</li></ul> | <ul><li>`activators` (array)</li><li>`actors` (array)</li><li>`serialization` (object)</li><li>`statics` (array)</li></ul> | <ul><li>`readOnlyHint`: true</li></ul> |
