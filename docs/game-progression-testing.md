...
2. Run `tests/mcp_discover.ps1 -OutputPath <run-directory>/initial-discovery.json` after the endpoint responds and before the first MCP tool operation. Inspect its independent-session `initialize`, `tools/list`, `resources/list`, and `prompts/list` results with the current menu/state before choosing an operation. Run `tests/prepare_morrowind_input.ps1` once after this initial observation and before the first operation; stop the exploration if input preparation fails. Start a `-WatchSeconds` discovery watcher before a state-changing operation when notification evidence is required; it records `notifications/*/list_changed` and the matching refreshed list. If no watcher is active, take a fresh discovery snapshot after the operation and record that notification evidence was unavailable.
