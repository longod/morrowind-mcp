...
function this.Test()
    ...
    unitwind:test("Squared normal threshold preserves slope and water classification", function()
        local grid = Grid(5, 1)
        local threshold = grid.maxSlopeNormalZSquared
        grid:SetSample(0, 0, 1, threshold * 0.5)
        grid:SetSample(1, 0, 1, threshold)
        grid:SetSample(2, 0, 1, (threshold + 1) / 2)
        grid:SetSample(3, 0, 1, 0)
        grid:SetSample(4, 0, -1, 1)
        unitwind:expect(grid:IsWalkable(grid:Index(0, 0))).toBe(false)
        unitwind:expect(grid:IsWalkable(grid:Index(1, 0))).toBe(true)
        unitwind:expect(grid:IsWalkable(grid:Index(2, 0))).toBe(true)
        unitwind:expect(grid:IsWalkable(grid:Index(3, 0))).toBe(false)
        unitwind:expect(grid:IsWalkable(grid:Index(4, 0))).toBe(false)
    end)
