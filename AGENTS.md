...
## 回答言語ルール
- ユーザーが回答言語を明示指定した場合は、その指定を優先する。
- 明示がなければ、会話自体の言語を優先する。
- それも曖昧なら、既定の返答言語を固定する。
...
- サーバー実行時に Lua から出力された mod data は `<paths.datafilesOverwriteDir>/MWSE/mods/morrowind-mcp` を確認する。
- [tests/mwmcp_config.ps1](tests/mwmcp_config.ps1) の `Paths.modDataDir` はこの物理パスを返す。エージェントが server output data を確認するときはこの値を優先して使用する。
- テストスクリプトが作成する sentinel ファイルは server output data ではない。
...
| `<paths.datafilesOverwriteDir>/MWSE/mods/morrowind-mcp` | サーバー実行時に Lua から出力された mod data の確認先 |

## 開発規約

- 不明なことがある場合、不明なまま作業を進めず、必ず質問すること
...
- サーバーからクライアントへの notifications は `MCP-Session-Id` ごとの SSE stream で送信する
- 同じ session から複数の SSE GET が来た場合は、新しい stream で古い stream を置き換える
