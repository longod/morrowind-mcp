...
- まず生成した summary の `status` と `mwse_analysis` を判定する。共通のartifactとseverityの規約は [test-run-summaries.md](../../../docs/test-run-summaries.md) を参照する。
- `passed` かつ `mwse_analysis.counts.fail` が `0` ならそこで終了し、ログ本文は調査しない。
