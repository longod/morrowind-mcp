"""MCP Inspector CLI adapter shared by Python test runners."""
...
def FormatToolArgument(value: Any) -> str:
    """Preserve Inspector scalar syntax while encoding structured values as JSON."""
...
def InvokeInspector(endpoint: str, operation: dict[str, Any], timeout_seconds: int) -> InspectorResponse:
    """Execute one MCP operation through the Inspector CLI."""
    arguments = ["npx.cmd", "--yes", "@modelcontextprotocol/inspector", "--cli", endpoint,
                 "--transport", "http", "--connect-timeout", str(timeout_seconds * 1000),
                 "--format", "json", "--method", operation["method"]]
    ...
    completed = subprocess.run(arguments, capture_output=True, check=False, encoding="utf-8", errors="replace",
    ...
    try:
        ...
        suffix = f" {detail}" if detail else ""
        raise InspectorError(f"Inspector did not return JSON.{suffix}") from error
